    ...
    <x-slot:footerFiles>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            btnGenerar.addEventListener("click", async (e) => {
                ...
                try {
                    const response = await fetch("{{ route('reporte.filtrar') }}", {
                        method: "POST",
                        headers: {
                    ...
                    if (data.length > 0) {
                        ...
                        tbody.innerHTML = data.map(f => `
                            <tr>
                                ...
                                <td>
                                    ${Array.isArray(f.usuarios) 
                                        ? f.usuarios.map(u => `<div>${u.nombre}</div>`).join('') 
                                        : f.usuarios}
                                </td>
                            </tr>`).join("");
                    } else {
            ...
            btnPdf.addEventListener("click", () => {
                ...
                window.open("{{ route('vehiculo.pdf') }}?" + params.toString(), "_blank");
            });
